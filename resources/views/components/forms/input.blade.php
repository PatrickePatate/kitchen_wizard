...
<div class="mb-4">
    ...
    @endif
    <input id="input-{{$name}}" type="{{$type}}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500  sm:text-sm {{$attributes->get('class')}}" @if($placeholder) placeholder="{{$placeholder}}" @endif @if($value) value="{{$value}}" @endif name="{{$name}}" value="{{ old($name) }}" @if($required)required @endif>
    @if($help)
